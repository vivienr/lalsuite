...
int create_td_waveform(REAL8TimeSeries ** h_plus, REAL8TimeSeries ** h_cross, struct params p)
{
    ...
    if (p.condition) {
        ...
        XLALSimInspiralChooseTDWaveform(h_plus, h_cross, p.m1, p.m2, p.s1x, p.s1y, p.s1z, p.s2x, p.s2y, p.s2z, p.distance, p.inclination, p.phiRef, 0., 0., 0., 1.0 / p.srate, p.f_min, p.fRef, p.lambda1, p.lambda2, 0., 0., p.waveFlags, p.nonGRparams, p.ampO, p.phaseO, p.approx);
        if (p.verbose)
            fprintf(stderr, "generation took %g seconds\n", (double)(clock() - timer_start) / CLOCKS_PER_SEC);
    } else {
        COMPLEX16FrequencySeries *htilde_plus = NULL;
        ...
        XLALSimInspiralChooseFDWaveform(&htilde_plus, &htilde_cross, p.m1, p.m2, p.s1x, p.s1y, p.s1z, p.s2x, p.s2y, p.s2z, p.distance, p.inclination, p.phiRef, 0., 0., 0., deltaF, p.f_min, 0.5 * p.srate, p.fRef, p.lambda1, p.lambda2, 0., 0., p.waveFlags, p.nonGRparams, p.ampO, p.phaseO, p.approx);
        if (p.verbose)
            fprintf(stderr, "generation took %g seconds\n", (double)(clock() - timer_start) / CLOCKS_PER_SEC);
